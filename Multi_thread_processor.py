...
from critical_functions import time_series_analysis, specilized_stat_analysis
...
def temperature_core(data_input, date_input, purpose, metric, forcast_horizon, tsf_dev_override, enhanced_diagnostic_mode,
    ...
    if True:
        ...
        thread_01 = threading.Thread(target=specilized_stat_analysis,args = (data_input, forecast_external_clone,
                                                                     developer_mode,enhanced_diagnostic_mode, metric, purpose))
        thread_02 = threading.Thread(target=error_metric,args = (data_input, forecast_external_clone,purpose, metric))
...
def humidity_core(data_input, date_input, purpose, metric, forcast_horizon, tsf_dev_override, enhanced_diagnostic_mode,
                     stat_analysis_mode, delay ,developer_mode, data_view_mode, column_name ,ghost_mode, confirmation_activation):
    assert(type(data_input) == list or type(data_input) == np.ndarray)
    assert(type(date_input) == list or type(date_input) == np.ndarray)
    assert(type(purpose) == str)
...
def rainfall_core(data_input, date_input, purpose, metric, forcast_horizon, tsf_dev_override, enhanced_diagnostic_mode,
                     stat_analysis_mode, delay ,developer_mode, data_view_mode, column_name ,ghost_mode, confirmation_activation):
    assert(type(data_input) == list or type(data_input) == np.ndarray)
    assert(type(date_input) == list or type(date_input) == np.ndarray)
    assert(type(purpose) == str)
